...
tab ca_sector, m
tab sector, m //12.4k only missing sectors
drop x sectorx
********************************************************************************
...
replace year = substr(cft_publdate,1,4) if missing(year) 
replace year = substr(ca_signdate,1,4) if missing(year) 
tab year if filter_ok, m
...
br lot_count tenderid ca_id lot_id ca_contract_value_num ca_lot_value_num pr_grant_currency pr_borrower_name w1_name if ca_id=="1201288"

*Getting missing lot values  from contract values if only 1 lot
replace ca_lot_value_num = ca_contract_value_num if filter_ok & missing(ca_lot_value_num) & lot_count==1
...
xtile ca_contract_value10 = ca_lot_value_num if filter_ok, nq(10)
replace ca_contract_value10=99 if missing(ca_lot_value_num)
...
egen w_id2=group(w1_name) if missing(w_id)
replace w_id2= 90526+w_id2 if !missing(w_id2)
tostring w_id2, replace
replace w_id2= "WB" +w_id2 if !missing(w_id2)
replace w_id2="" if w_id2=="WB."
gen w_id3 = w_id
replace w_id3=w_id2 if missing(w_id3)
br w_id w_id2 w_id3 w1_name if missing(w_id) & !missing(w1_name)
drop w_id w_id2
rename w_id3 w_id
********************************************************************************
