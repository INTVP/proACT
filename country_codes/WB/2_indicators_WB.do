...
replace iso = w1_country if missing(iso)
...
egen w_yam=sum(ca_lot_value_num) if filter_ok==1 & w_id!=. & year!=., by (w_id year) 
lab var w_yam "By Winner-year: Spending amount"

egen proa_w_yam=sum(ca_lot_value_num) if filter_ok==1 & anb_id!=. & w_id!=. & year!=., by(anb_id w_id year)
...
egen proa_yam=sum(ca_lot_value_num) if filter_ok==1 & anb_id!=. & year!=., by(anb_id year) 
...
preserve
    ...
    rename ca_lot_value_num ca_contract_value //bid price variable
    keep if filter_ok==1 
