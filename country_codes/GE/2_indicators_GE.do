...
egen proa_yam=sum(ca_contract_value) if filter_ok==1 & !missing(buyer_id) & !missing(tender_year), by(buyer_id tender_year) 
...
egen filter_proay = tag(buyer_id tender_year) if filter_ok==1 &  !missing(buyer_id)  & !missing(tender_year)
...
do "${utility_codes}/gen_bidder_non_local.do" "`country'" buyer_city_original bidder_city
********************************************************************************
*Benford's
...
do "${utility_codes}/cri.do" singleb corr_proc_bi corr_submp_bi corr_decp nocft taxhav2 proa_ycsh4 corr_ben_bi 
rename cri cri_ge
