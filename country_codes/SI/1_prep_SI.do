...
*Data 
import delimited using  $utility_data/country/AT/starting_data/SI_data.csv, encoding(UTF-8) clear
********************************************************************************

*Filter ok main - not missing bidder name
gen filter_ok=0
replace filter_ok =1 if !missing(bidder_name) 
*Not including opentender

tab filter_ok, m
tab opentender, m
count if missing(bidder_name)
gen miss_name=missing(bidder_name) 
tab miss_name opentender, m
drop miss_name
...
tab source if miss_frame==1 & filter_ok //missing framework agg var if source is national - so it means we don't know if its a framework agreement or not mainly this national source: "http://www.enarocanje.si "
...
*Estimated: tender_estimatedprice, lot_estimatedprice
*Actual: tender_finalprice, bid_price
*Fix missing bid_price from bid_digiwhist_price

*Fix currency
tab curr if filter_ok, m
drop if !inlist(currency,"EUR","") //23 obs
...
********************************************************************************
use $country_folder/SI_wip.dta,clear
...
gen bid_price_ppp = bid_digiwhist_price/ppp_eur 
...
replace tender_year = real(substr(tender_publications_firstcallfor,1,4)) if missing(tender_year) & filter_ok
replace tender_year = real(substr(tender_publications_firstdcontra,1,4)) if missing(tender_year) & filter_ok
******************************************
...
gen buyer_nuts_handle1=substr(buyer_nuts,1,4)
gen anb_location1=buyer_nuts_handle1 if regex(buyer_nuts_handle1,"^SI") 
replace anb_location1="EXT" if regex(buyer_nuts_handle1,"^SI")==0 & !missing(buyer_nuts_handle1)
