...
*Data 
import delimited using  $utility_data/country/AT/starting_data/BG_data.csv , encoding(UTF-8) clear
********************************************************************************
...
replace filter_ok = 0 if curr=="GBP"
replace filter_ok = 0 if curr=="USD"
...
keep if inlist(countryname,"Bulgaria")
drop if ppp==.
keep year ppp
save $country_folder/ppp_data_bgn.dta,replace
...
merge m:1 year using $country_folder/ppp_data_bgn.dta
drop if _m==2
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=0.6926209 if missing(ppp) & year==2020 //used 2019
*br year ppp if _m==3
drop _m year
rename ppp ppp_bgn
...
replace bid_price_ppp = bid_price/ppp_bgn if currency=="BGN"
replace bid_price_ppp = bid_price/ppp_bgn if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace tender_finalprice_ppp = tender_finalprice/ppp_bgn if currency=="BGN"
replace tender_finalprice_ppp = tender_finalprice/ppp_bgn if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_bgn if currency=="BGN"
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_bgn if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_bgn if currency=="BGN"
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_bgn if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace curr_ppp = currency if !inlist(currency,"EUR","BGN")
...
tab market_id, m-8) clear
************************************

*drop anb_loc loc_country
gen anb_loc = substr(tender_addressofimplementation_n,1,3)
replace anb_loc = substr(buyer_nuts,1,3) if missing(tender_addressofimplementation_n)
tab anb_loc
gen loc_country = substr(anb_loc,1,2)
tab loc_country
replace anb_loc = "FOREIGN" if loc_country!="BG"
tab anb_loc
replace anb_loc = "NA" if anb_loc=="BG"
replace anb_loc = "NA" if anb_loc=="BGZ"
tab anb_loc
*replace buyer_type="NA" if missing(buyer_type)
encode anb_loc, gen(buyer_loc)
drop anb_loc
tab buyer_loc, m
rename buyer_loc anb_loc
************************************
