...
decode w_id, gen(bidder_masterid)
...
decode anb_loc, gen(buyer_city)
decode anb_county, gen(buyer_dept)
replace buyer_city = "" if buyer_city=="."
replace buyer_dept = "" if buyer_dept=="."
do "${utility_codes}/gen_is_capital.do" "`country'" buyer_city buyer_dept

*Generate bidder is non local {`country', buyer_city bidder_city, buyer_nuts bidder_nuts}
decode w_loc, gen(bidder_city)
decode w_county, gen(bidder_dept)
replace bidder_city = "" if bidder_city=="."
replace bidder_dept = "" if bidder_dept=="."
...
rename buyer_id buyer_id_original
...
cap drop cri_col
do "${utility_codes}/cri.do" singleb nocft corr_proc_bi corr_ben_bi taxhav2 corr_rdelay2_bi proa_ycsh5 roverrun2
rename cri cri_col
