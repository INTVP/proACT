...
*Data
use $country_folder/HR_wip.dta, clear
********************************************************************************
...
*Level 1 risk - NEGOTIATED_WITHOUT_PUBLICATION, NA

label list ca_procedure2
gen corr_proc=0
*replace corr_proc=0 if inlist(ca_procedure,1,3,7,8,11,12) 
replace corr_proc=1 if inlist(ca_procedure,4,2)
...
sum submp if filter_ok  //mean 37.8 days
xtile submp10=submp if filter_ok==1, nquantiles(10)
replace submp10=99 if submp==.
tabstat submp, by(submp10) stat(min mean max)

logit singleb ib5.submp10 i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
gen corr_submp=0
replace corr_submp=1 if inlist(submp5,1)
...
sum decp if filter_ok //mean: 108.4 days
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="HR" | bidder_country==""
tab fsuppl, missing
...
save $country_folder/HR_wip.dta, replace
...
mean MAD if filter_ok //.1392 mean
...
********************************************************************************
save $country_folder/HR_wb_1020.dta, replace
...
do $utility_codes/cri.do singleb corr_proc corr_submp corr_decp_bi nocft taxhav proa_ycsh4
rename cri cri_hr

sum cri_hr if filter_ok==1
hist cri_hr if filter_ok==1, title("CRI HR, filter_ok")
hist cri_hr if filter_ok==1, by(tender_year, noiy title("CRI HR (by year), filter_ok")) 
********************************************************************************
