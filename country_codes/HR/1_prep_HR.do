...
*Data 
import delimited using  $utility_data/country/AT/starting_data/HR_data.csv, encoding(UTF-8) clear
********************************************************************************
...
keep if inlist(countryname,"Croatia")
drop if ppp==.
keep year ppp
save $country_folder/ppp_data_hrk.dta,replace
...
merge m:1 year using $country_folder/ppp_data_hrk.dta
drop if _m==2
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=3.28176 if missing(ppp) & year==2020 //used 2019
br year ppp if _m==3
drop _m year
rename ppp ppp_hrk
...
replace bid_price_ppp = bid_price/ppp_hrk if currency=="HRK"
...
replace tender_finalprice_ppp = tender_finalprice/ppp_hrk if currency=="HRK"
replace tender_finalprice_ppp = tender_finalprice/ppp_hrk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_hrk if currency=="HRK"
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_hrk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_hrk if currency=="HRK"
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_hrk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace curr_ppp = currency if !inlist(currency,"HRK","EUR")
********************************************************************************
