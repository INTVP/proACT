...
*Data 
import delimited using  $utility_data/country/AT/starting_data/PL_data.csv, encoding(UTF-8) clear
********************************************************************************
...
keep if inlist(currency,"EUR","PLN","")
save $country_folder/PL_wip.dta, replace
********************************************************************************
...
use $utility_data/wb_ppp_data.dta, clear
keep if inlist(countryname,"Poland")
drop if ppp==.
keep year ppp
save $country_folder/ppp_data_pln.dta, replace
********************************************************************************
...
merge m:1 year using $country_folder/ppp_data_pln.dta
drop if _m==2
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=1.749863 if missing(ppp) & year==2020 //used 2019
br year ppp if _m==3
drop _m year
rename ppp ppp_pln
************************************
...
replace lot_estimatedprice_ppp=lot_estimatedprice/ppp_pln  if currency=="PLN"
...
gen anb_location1=buyer_nuts if regex(buyer_nuts,"^PL") 
replace anb_location1="EXT" if regex(anb_location1,"^PL")==0 & !missing(buyer_nuts)
