...
*Data 
import delimited using  $utility_data/country/AT/starting_data/IS_data.csv, encoding(UTF-8) clear
********************************************************************************

*Filter ok main - not missing bidder name

tab bid_iswinning , m //no losing
tab bid_iscons , m //only 49 obs 
tab tender_isframe , m //
tab opentender , m //all
tab source , m //TED + National
...
gen miss_con_url=missing(tender_publications_lastcontract)
tab miss_bidder, m
tab miss_con_url , m
tab miss_bidder miss_con_url, m
bys source: tab miss_bidder, m

drop miss_bidder miss_con_url
************************************
...
*Actual:
*tender_finalprice, bid_price, bid_digiwhist_price 
*Fix currency
...
use $utility_data/wb_ppp_data.dta, clear
keep if inlist(countryname,"Iceland")
drop if ppp==.
keep year ppp
save $country_folder/ppp_data_isl.dta, replace
********************************************************************************
...
gen year = tender_year
merge m:1 year using $country_folder/ppp_data_isl.dta
drop if _m==2
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=136.6584 if missing(ppp) & year==2020 //used 2019
br year ppp if _m==3
drop _m year
rename ppp ppp_isl
************************************
...
tab currency, m
sort currency
br bid_digiwhist_price source  bid_price currency if  filter_ok
replace bid_digiwhist_price= bid_price if missing(bid_digiwhist_price) & currency==("ISK")
replace bid_digiwhist_price=tender_finalprice if missing(bid_digiwhist_price) & tender_lotscount==1
replace bid_digiwhist_price=tender_digiwhist_price if missing(bid_digiwhist_price) & tender_lotscount==1

gen bid_price_ppp=.
replace bid_price_ppp = bid_digiwhist_price/ppp_eur if currency=="EUR"
replace bid_price_ppp = bid_digiwhist_price/ppp_isl if currency=="ISK"

replace lot_estimatedprice=tender_estimatedprice if missing(lot_estimatedprice) & tender_lotscount==1
gen lot_estimatedprice_ppp = .
replace lot_estimatedprice_ppp=lot_estimatedprice/ppp_eur  if currency=="EUR"
replace lot_estimatedprice_ppp=lot_estimatedprice/ppp_isl  if currency=="ISK"
...
gen curr_ppp = ""
replace curr_ppp = "International Dollars" if !missing(bid_price_ppp)  | !missing(lot_estimatedprice_ppp)
...
gen anb_location1=buyer_nuts if regex(buyer_nuts,"^IS") 
replace anb_location1="EXT" if regex(anb_location1,"^IS")==0 & !missing(buyer_nuts)
