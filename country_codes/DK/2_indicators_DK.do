...
*Data
use $country_folder/DK_wip.dta, clear
********************************************************************************
...
*Level 1 risk - NEGOTIATED_WITHOUT_PUBLICATION, OUTRIGHT_AWARD
*Level 2 risk -  , Missing

label list ca_procedure2
cap drop corr_proc
gen corr_proc=.
replace corr_proc=0 if inlist(ca_procedure,1,2,4,6,7,9) 
replace corr_proc=1 if inlist(ca_procedure,5,8)
replace corr_proc=2 if inlist(ca_procedure,3)
...
sum submp if filter_ok  //mean 82.8 days
tabstat submp if filter_ok,stat(mean median) //median 44
...
cap drop corr_submp
gen corr_submp=0
replace corr_submp=1 if inlist(submp10,1,2,3,4,5,6,99)
...
*Not valid
drop yescft nocft nocft2 corr_nocft
*Not using nocft
********************************************************************************
...
tabstat decp if filter_ok,stat(n min mean max median)  //mean: 89.5 days median: 75
...
*Med risk 3-5, NA
*compared to median
logit singleb ib5.decp10 i.corr_submp i.corr_proc i.anb_location i.ca_contract_value10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*High risk 1
*Med risk 2

cap drop corr_decp
gen corr_decp=0
replace corr_decp=1 if inlist(decp10,2,3,4,5)
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="DK" | bidder_country==""
tab fsuppl, missing
...
save $country_folder/DK_wip.dta, replace
...
tabstat MAD if filter_ok, stat(mean median) //mean .0228596  median .0206239
...
*nothing works
*compared to median
logit singleb ib5.ben  i.taxhav3 i.corr_proc  i.corr_decp i.corr_submp i.anb_location i.ca_contract_value10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*nothing works
...
cap drop corr_ben
gen corr_ben=0
replace corr_ben=2 if inlist(ben,4,5)
...
do $utility_codes/cri.do singleb corr_proc_bi corr_subm corr_decp_bi proa_ycsh4 taxhav2 corr_ben_bi
rename cri cri_dk

sum cri_dk if filter_ok==1
hist cri_dk if filter_ok==1, title("CRI DK, filter_ok")
hist cri_dk if filter_ok==1, by(tender_year, noiy title("CRI DK (by year), filter_ok")) 
********************************************************************************

save $country_folder/DK_wb_2011.dta, replace
********************************************************************************
*END