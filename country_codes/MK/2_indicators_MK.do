...
*Data
use $country_folder/MK_wip.dta, clear
********************************************************************************
...
logit singleb ib6.ca_procedure i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
*Level 1 risk - RESTRICTED

label list ca_procedure2
gen corr_proc=0
*replace corr_proc=0 if inlist(ca_procedure,1,3,7,8,11,12) 
replace corr_proc=1 if inlist(ca_procedure,7)
...
sum submp if filter_ok  //mean 27.62 days
xtile submp10=submp if filter_ok==1, nquantiles(10)
replace submp10=99 if submp==.
tabstat submp, by(submp10) stat(min mean max)

logit singleb ib6.submp10 i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
logit singleb ib3.submp5 i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
logit singleb ib5.submp5 i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base


gen corr_submp=0
replace corr_submp=1 if inlist(submp5,5,99)
replace corr_submp=2 if inlist(submp5,1)
...
sum decp if filter_ok //mean: 49.65 days
tabstat decp, by(decp10) stat(min mean max)

logit singleb ib6.decp10 i.corr_submp i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*<=6
...
gen corr_decp=0
replace corr_decp=1 if inlist(decp10,2,3,4,99)
...
logit singleb i.nocft i.corr_decp  i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
logit singleb i.nocft#i.corr_proc i.corr_decp  i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*Valid when interacting with corr_proc 
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="MK" | bidder_country==""
tab fsuppl, missing
...
logit singleb i.taxhav i.nocft#i.corr_proc i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
logit singleb i.taxhav2 i.nocft#i.corr_proc i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*Second one is valid, but there are not many cases 
...
save $country_folder/MK_wip.dta, replace
...
save buyers_benford.dta, replace
*/
************************************************
...
xtile ben=MAD if filter_ok==1, nquantiles(5)
replace ben=99 if MAD==. 
mean MAD if filter_ok //.0225647 mean
tabstat MAD, by(ben) stat(min max)
logit singleb ib3.ben i.nocft i.corr_decp i.corr_proc i.anb_type i.ca_type i.tender_year i.market_id  i.cvalue10  if filter_ok , base
...
logit singleb i.nocft#i.corr_proc i.taxhav2 i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
reg proa_ycsh singleb i.taxhav2 i.nocft i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok  & proa_ynrc>4 & proa_ynrc!=., base
outreg2 using se_valid2.doc

*Calc from buyer side more valid than calc from winner side. 
********************************************************************************
save $country_folder/MK_wb_1020.dta, replace

********************************************************************************
use $country_folder/MK_wb_1020.dta, replace
*CRI generation

sum singleb corr_proc corr_submp corr_decp nocft taxhav proa_ycsh if filter_ok==1
tab singleb, m
tab corr_proc, m 
...
do $utility_codes/cri.do singleb corr_proc corr_submp_bi corr_decp_bi nocft taxhav2 proa_ycsh4
rename cri cri_mk

sum cri_mk if filter_ok==1
hist cri_mk if filter_ok==1, title("CRI MK, filter_ok")
hist cri_mk if filter_ok==1, by(tender_year, noiy title("CRI HR (by year), filter_ok")) 
********************************************************************************
