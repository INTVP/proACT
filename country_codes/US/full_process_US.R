...
#Load Data
us_award_data_filtered <- read.csv(paste0(utility_data,"/country/US/starting_data/us_award_data_filtered.dta"))
...
us_award_data_filtered$corr_proc_old <- us_award_data_filtered$corr_proc

summary(us_award_data_filtered$corr_proc_old)
us_award_data_filtered$corr_proc_old <- as.numeric(us_award_data_filtered$corr_proc_old)-1
summary(us_award_data_filtered$corr_proc_old)

us_award_data_filtered$corr_solic <- as.numeric(us_award_data_filtered$corr_solic)-1
summary(us_award_data_filtered$corr_solic)

us_award_data_filtered$corr_proc <- as.factor((us_award_data_filtered$corr_proc_old)/2+(us_award_data_filtered$corr_solic)/2)
...
us_award_data_filtered <- us_award_data_filtered %>% group_by(parent_award_agency_id) %>% mutate(ncon=n())
#length(us_award_data_filtered$buyer_id[us_award_data_filtered$buyer_id=="buyer_id_156"])
#summary(us_award_data_filtered$ncon[us_award_data_filtered$buyer_id=="buyer_id_156"])
...
us_benford <- us_award_data_filtered %>% 
  filter(miss_buyerid==0&miss_cv==0&con100==1) %>% 
  dplyr::select(parent_award_agency_id,obligated_amount) #%>% 
#rename(ca_value=obligated_amount)

#readr::write_csv(us_benford, paste0(country_folder,"/buyers_for_R.csv"))
...
buyers_proc_summary <- us_benford %>%
...
buyers_proc_summary <- buyers_proc_summary %>%
  ...
  summarise(MAD_conformitiy = benford_f(vector_in=obligated_amount,"MAD.conformity"),
            MAD = benford_f(vector_in=obligated_amount,"MAD"))


#Run the Benford script

us_award_data_filtered <- merge(us_award_data_filtered,buyers_proc_summary, by='parent_award_agency_id',all.x=TRUE)

us_award_data_filtered$MAD_conformitiy <- as.factor(us_award_data_filtered$MAD_conformitiy)
...
us_award_data_filtered$MAD10[!is.na(us_award_data_filtered$MAD)] <- factor(cut(rank(us_award_data_filtered$MAD[!is.na(us_award_data_filtered$MAD)], ties.method = "first"), quantile(rank(us_award_data_filtered$MAD[!is.na(us_award_data_filtered$MAD)], ties.method = "first"), prob = seq(0, 1, length = 11), type = 5,na.rm=FALSE), include.lowest=TRUE,labels=c(1:10),exclude=NULL))
...
buyer_bidder <- as.data.frame(us_award_data_filtered %>% 
                                group_by(awarding_office_code,recipient_duns,year) %>%
                                summarise(agg_value = sum(obligated_amount))) %>% 
  filter(is.na(year)==FALSE)

buyer <- as.data.frame(us_award_data_filtered %>% 
                         group_by(awarding_office_code,year) %>%
                         summarise(agg_value = sum(obligated_amount))) %>% 
  filter(is.na(year)==FALSE)

buyer_conc_df <- merge(buyer_bidder,buyer,by=c('awarding_office_code','year'),all.x=TRUE)

buyer_conc_df$share <- buyer_conc_df$agg_value.x/buyer_conc_df$agg_value.y
buyer_conc_df <- buyer_conc_df %>% select(awarding_office_code,recipient_duns,year, share)

us_award_data_filtered_try <- merge(us_award_data_filtered,buyer_conc_df,by=c('awarding_office_code','recipient_duns','year'),all.x=TRUE)
...
list.vars <- c("awarding_office_code",  "recipient_duns",  "year",  "parent_award_agency_id",  "award_id_piid",  "parent_award_agency_name",  "parent_award_id",  "obligated_amount",  "current_total_value_of_award",  "awarding_agency_code",  "awarding_agency_name",  "awarding_sub_agency_code",  "awarding_sub_agency_name",  "awarding_office_name",  "recipient_name",  "cage_code",  "recipient_parent_name",  "recipient_parent_duns",  "recipient_country_code",  "recipient_country_name",  "recipient_address_line_1",  "recipient_address_line_2",  "recipient_city_name",  "recipient_state_code",  "recipient_state_name",  "recipient_zip_4_code",  "recipient_congressional_district",  "recipient_phone_number",  "recipient_fax_number",  "primary_place_of_performance_country_code",  "primary_place_of_performance_country_name",  "primary_place_of_performance_city_name",  "primary_place_of_performance_county_name",  "primary_place_of_performance_state_code",  "primary_place_of_performance_state_name",  "primary_place_of_performance_zip_4",  "primary_place_of_performance_congressional_district",  "award_type",  "award_description",  "product_or_service_code",  "product_or_service_code_description",  "naics_code",  "naics_description",  "country_of_product_or_service_origin_code",  "country_of_product_or_service_origin",  "solicitation_procedures_code",  "solicitation_procedures",  "number_of_offers_received",  "singleb",  "prop_bidnr",  "corr_proc",  "corr_solic",  "nocft",  "overrun",  "delay",  "commercial_item",  "state_code",  "unid_foreign_comp",  "w_foreign",  "msector",  "lcv",  "cv5",  "corr_overrun",  "corr_delay",  "corr_ben",  "share",'solicitation_procedures_code','extent_competed_code')

export <- us_award_data_filtered_try %>% select(list.vars)
export[export==''] <- NA
...
export$export_tender_proceduretype <- paste0(as.character(export$extent_competed_code ),"-", as.character(export$solicitation_procedures_code))
...
list.of.divisions <- read_delim('cpv_divisions.txt',col_types = cols(.default = col_character()), delim = '\t')
...
for (index in 1:nrow(list.of.divisions)) {
  export$tender_cpvs[grepl(pattern = list.of.divisions$REGEX[index],
                           ...
                           perl = T)] <- list.of.divisions$CPV[index]
}
...
export$singleb <- as.numeric(export$singleb)-1
table(export$singleb)#0,1
...
library(dplyr)
export_vf <- export %>% dplyr::select(vars)

length(is.na(export_vf$title))
summary(is.na(export_vf$title))

write.csv(export_vf, paste0(country_folder,"/US_mod.csv"),fileEncoding ="UTF-8")