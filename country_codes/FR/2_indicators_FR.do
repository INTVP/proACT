...
*Data
use $country_folder/FR_wip.dta, clear
********************************************************************************
...
*Level 1 risk - OPEN, COMPETITIVE_DIALOG, INOVATION_PARTNERSHIP, NEGOTIATED, RESTRICTED
*Level 1 risk - NEGOTIATED_WITH_PUBLICATION
*Level 2 risk - NEGOTIATED_WITHOUT_PUBLICATION, OUTRIGHT_AWARD, NA

label list ca_procedure2
gen corr_proc=0
*replace corr_proc=0 if inlist(ca_procedure,1,3,7,8,11,12) 
replace corr_proc=1 if inlist(ca_procedure,6)
replace corr_proc=2 if inlist(ca_procedure,5,8,3) 
...
sum submp if filter_ok  //mean 73.87 days
...
*drop corr_submp
gen corr_submp=0
replace corr_submp=1 if inlist(submp10,1,2,3)
replace corr_submp=2 if inlist(submp10,9,10,99)
...
*Something weird is going on, for some reason there is no big difference between the two risky categories
********************************************************************************
...
sum decp if filter_ok //mean: 131.5 days
...
gen corr_decp=0
replace corr_decp=1 if inlist(decp10,3,4,99)
...
*/
logit singleb i.nocft i.corr_decp  i.corr_submp i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*drop nocft
*rename corr_nocft nocft
********************************************************************************
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="FR" | bidder_country==""
tab fsuppl, missing
...
save $country_folder/FR_wip.dta, replace
...
logit singleb i.corr_ben i.taxhav2 i.nocft i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*Does not work

xtile ben=MAD if filter_ok==1, nquantiles(10)
replace ben=99 if MAD==. 
mean MAD if filter_ok //.0215132 mean
tabstat MAD, by(ben) stat(min max)
logit singleb ib6.ben i.nocft i.corr_decp i.corr_proc i.anb_type i.ca_type i.tender_year i.market_id  i.cvalue10  if filter_ok , base
...
logit singleb i.nocft i.taxhav2 i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
sum singleb corr_proc corr_submp corr_decp nocft taxhav2 proa_ycsh if filter_ok==1
tab singleb, m
...
do $utility_codes/cri.do singleb corr_proc_bi corr_submp_bi corr_decp_bi nocft taxhav2 proa_ycsh4
rename cri cri_fr

sum cri_fr if filter_ok==1
hist cri_fr if filter_ok==1, title("CRI FR, filter_ok")
hist cri_fr if filter_ok==1, by(tender_year, noiy title("CRI FR (by year), filter_ok")) 
********************************************************************************
