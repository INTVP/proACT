...
*Data 
import delimited using  $utility_data/country/AT/starting_data/SE_data.csv, encoding(UTF-8) clear
********************************************************************************
...
keep if inlist(countryname,"Sweden")
drop if ppp==.
keep year ppp
save $country_folder/ppp_data_skk.dta,replace
...
merge m:1 year using $country_folder/ppp_data_skk.dta
drop if _m==2
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=8.745862 if missing(ppp) & year==2020 //used 2019
br year ppp if _m==3
drop _m year
rename ppp ppp_skk
...
tab year if _m==1, m //2020 no ppp data
tabstat ppp, by(year)
replace ppp=8.684051 if missing(ppp) & year==2020 //used 2019
br year ppp if _m==3
drop _m year
rename ppp ppp_eur
...
replace bid_price_ppp = bid_price/ppp_skk if currency=="SKK"
replace bid_price_ppp = bid_price/ppp_skk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace tender_finalprice_ppp = tender_finalprice/ppp_skk if currency=="SKK"
replace tender_finalprice_ppp = tender_finalprice/ppp_skk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_skk if currency=="SKK"
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_skk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_skk if currency=="SKK"
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_skk if !inlist(source,"http://data.europa.eu/","http://ted.europa.eu") & missing(currency)
...
replace curr_ppp = currency if !inlist(currency,"SKK","EUR")
...
save $country_folder/SI_wip.dta , replace
********************************************************************************
*END
