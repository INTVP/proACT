...
*Data 
import delimited using  $utility_data/country/AT/starting_data/NO_data.csv, encoding(UTF-8) clear
********************************************************************************
...
sum tender_finalprice if tender_finalprice==0 
*no such cases
...
************************************
save $country_folder/NO_wip.dta, replace

********************************************************************************
use $utility_data/wb_ppp_data.dta, clear
keep if countrycode == "NOR"
drop if ppp==.
...
********************************************************************************
use $country_folder/NO_wip.dta, clear
...
gen tender_finalprice_ppp = tender_finalprice/ppp_nok if currency=="NOK" | missing(currency)
gen bid_price_ppp =  bid_price/ppp_nok if currency=="NOK" | missing(currency)
gen ca_contract_value_ppp =  ca_contract_value/ppp_nok if currency=="NOK" | missing(currency)
gen tender_estimatedprice_ppp = tender_estimatedprice/ppp_nok if currency=="NOK" | missing(currency)
gen lot_estimatedprice_ppp = lot_estimatedprice/ppp_nok if currency=="NOK" | missing(currency)
gen lot_updatedprice_ppp = lot_updatedprice/ppp_nok if currency=="NOK" | missing(currency)
drop ppp

replace tender_finalprice_ppp = tender_finalprice/ppp_eur if currency=="EUR" | missing(currency)
replace bid_price_ppp =  bid_price/ppp_eur if currency=="EUR" | missing(currency)
replace ca_contract_value_ppp =  ca_contract_value/ppp_eur if currency=="EUR" | missing(currency)
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_eur if currency=="EUR" | missing(currency)
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_eur if currency=="EUR" | missing(currency)
replace lot_updatedprice_ppp = lot_updatedprice/ppp_eur if currency=="EUR" | missing(currency)
...
replace buyer_nuts="" if anb_nuts2!="NO"
