...
sum submp if filter_ok  //mean 92 days
...
gen corr_submp2=0
replace corr_submp2=1 if inlist(submp10,1,2,3,10)
replace corr_submp2=99 if submp10==99
tab corr_submp2 if filter_ok
logit singleb i.corr_submp2 i.corr_proc ca_contract_value100 i.buyer_location i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
tabstat submp, by(submp10) stat(min mean max)
...
sum decp if filter_ok //mean: 145 days
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="MT" | bidder_country==""
tab fsuppl, missing
...
hist cri_mt if filter_ok==1, title("CRI MT, filter_ok")
