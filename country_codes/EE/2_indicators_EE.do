...
*Data
use $country_folder/EE_wip.dta, clear
********************************************************************************
...
*Level 1 risk - OTHER, OUTRIGHT_AWARD, NEGOTIATED
*Level 2 risk - NEGOTIATED_WITHOUT_PUBLICATION, CONCESSION
label list ca_procedure2
gen corr_proc=.
replace corr_proc=0 if inlist(ca_procedure,1,3,7,8,11,12) 
replace corr_proc=1 if inlist(ca_procedure,9,10,5)
...
sum submp if filter_ok  //mean 30 days
...
gen corr_submp=0
replace corr_submp=1 if inlist(submp10,3,4,5,6)
replace corr_submp=2 if inlist(submp10,1,2)
...
sum decp if filter_ok //mean: 41.6 days
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="EE" | bidder_country==""
tab fsuppl, missing
...
logit singleb i.corr_ben i.nocft i.taxhav i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
reg proa_ycsh singleb i.taxhav i.corr_ben i.nocft i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok  & proa_ynrc>4 & proa_ynrc!=., base
outreg2 using se_valid2.doc

*Calc from buyer side more valid than calc from winner side. 
********************************************************************************

save $country_folder/EE_wb_1020.dta, replace
********************************************************************************
*CRI calculation

sum singleb corr_proc corr_submp corr_decp nocft taxhav corr_ben proa_ycsh if filter_ok==1
tab singleb, m
...
do $utility_codes/cri.do singleb corr_proc_bi corr_submp_bi corr_decp_bi nocft taxhav corr_ben_bi proa_ycsh4
rename cri cri_ee

sum cri_ee if filter_ok==1
hist cri_ee if filter_ok==1, title("CRI EE, filter_ok")
hist cri_ee if filter_ok==1, by(tender_year, noiy title("CRI EE (by year), filter_ok")) 
********************************************************************************
