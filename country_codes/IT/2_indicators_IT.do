...
*Level 1 risk - OPEN, COMPETITIVE_DIALOG, NEGOTIATED, NEGOTIATED_WITH_PUBLICATION
*Level 1 risk - RESTRICTED, INOVATION_PARTNERSHIP
*Level 2 risk - NEGOTIATED_WITHOUT_PUBLICATION, OUTRIGHT_AWARD, NA

label list ca_procedure2
gen corr_proc=0
*replace corr_proc=0 if inlist(ca_procedure,1,3,7,8,11,12) 
replace corr_proc=1 if inlist(ca_procedure,2,9)
...
gen corr_submp=0
replace corr_submp=1 if inlist(submp10,2,3,9,10)
replace corr_submp=2 if inlist(submp10,1,99)
...
*Valid for corr_proc == 2

logit singleb i.nocft##i.corr_proc i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
*Valid for both corr_proc risky categories
...
logit singleb i.corr_nocft i.corr_decp  i.corr_submp i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
drop nocft
...
gen fsuppl=1 
replace fsuppl=0 if bidder_country=="IT" | bidder_country==""
tab fsuppl, missing
...
mean MAD if filter_ok //.015276 mean
...
logit singleb i.nocft i.taxhav i.corr_decp i.corr_submp i.corr_proc i.cvalue10 i.anb_type i.ca_type i.tender_year i.market_id  if filter_ok, base
...
do $utility_codes/cri.doingleb singleb corr_proc_bi corr_submp_bi corr_decp_bi corr_nocft_bi taxhav proa_ycsh4
rename cri cri_it

sum cri_it if filter_ok==1
hist cri_it if filter_ok==1, title("CRI IT, filter_ok")
hist cri_it if filter_ok==1, by(tender_year, noiy title("CRI IT (by year), filter_ok")) 
********************************************************************************

save $country_folder/IT_wb_1020.dta, replace
********************************************************************************
*END