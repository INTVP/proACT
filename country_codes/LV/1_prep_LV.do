...
keep if inlist(countryname,"Latvia")
drop if ppp==.
keep year ppp
save "${country_folder}/ppp_data_lv.dta", replace
...
merge m:1 year using "${country_folder}/ppp_data_lv.dta"
drop if _m==2
// tabstat ppp, by(year)
...
gen tender_finalprice_ppp = tender_finalprice/ppp_eur if currency=="EUR" 
gen bid_price_ppp =  bid_price/ppp_eur if currency=="EUR" 
gen ca_contract_value_ppp =  ca_contract_value/ppp_eur if currency=="EUR" 
gen tender_estimatedprice_ppp = tender_estimatedprice/ppp_eur if currency=="EUR" 
gen lot_estimatedprice_ppp = lot_estimatedprice/ppp_eur if currency=="EUR"
gen lot_updatedprice_ppp = lot_updatedprice/ppp_eur if currency=="EUR"

replace tender_finalprice_ppp = tender_finalprice/ppp_lv if currency=="LVL" 
replace bid_price_ppp =  bid_price/ppp_lv if currency=="LVL" 
replace ca_contract_value_ppp =  ca_contract_value/ppp_lv if currency=="LVL" 
replace tender_estimatedprice_ppp = tender_estimatedprice/ppp_lv if currency=="LVL" 
replace lot_estimatedprice_ppp = lot_estimatedprice/ppp_lv if currency=="LVL" 
replace lot_updatedprice_ppp = lot_updatedprice/ppp_lv if currency=="LVL" 
